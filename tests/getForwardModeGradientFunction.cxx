...
struct TFELMathEnzymeGetDerivative final : public tfel::tests::TestCase {
  TFELMathEnzymeGetDerivative()
      : tfel::tests::TestCase("TFEL/Math/Enzyme", "TFELMathEnzymeGetDerivative") {
...
private:
 ...
 void test2() {
   ...
   const auto ds = getForwardModeGradientFunction<0>([](const Stensor& v) {
     return trace(v);
   });
   TFEL_TESTS_ASSERT(abs(ds(s) - Stensor::Id()) < eps);
   const auto ds2 =
       getForwardModeGradientFunction<0>([](const Stensor& v) -> double { return sigmaeq(v); });
 ...
 void test4() {
  ...
  void test6() {
    ...
    const auto stiffness = getForwardModeGradientFunction<0, 0>(hooke_potential);
...
TFEL_TESTS_GENERATE_PROXY(TFELMathEnzymeGetDerivative, "TFELMathEnzymeGetDerivative");
