/*!
 * \file   tests/getForwardModeDerivativeFunction.cxx
 * \brief
...
struct TFELMathEnzymeGetForwardModeDerivativeFunction final
 ...
 private:
  void test1() {
    ...
    // first derivative
    auto df = getForwardModeDerivativeFunction<0>(f);
    TFEL_TESTS_ASSERT(std::abs(df(2) - 12) < eps);
    // second derivative
    auto d2f = getForwardModeDerivativeFunction<0, 0>(f);
  ...
  void test3() {
    ...
    const auto stiffness = getForwardModeDerivativeFunction<0, 0>(Phi);
  ...
  void test5() {
    ...
    const auto dvp = getForwardModeDerivativeFunction<0>(first_eigen_value)(s);
    const auto [vp, m] =
...
TFEL_TESTS_GENERATE_PROXY(TFELMathEnzymeGetForwardModeDerivativeFunction,
                          "TFELMathEnzymeGetForwardModeDerivativeFunction");
...
int main() {
  ...
  m.addXMLTestOutput("tfel-math-enzyme-getForwardModeDerivativeFunction.xml");
