/*!
 * \file   TFEL/Math/Enzyme/autodiff.hxx
 * \brief
 * \author Thomas Helfer
 * \date   19/08/2024
 */

#ifndef LIB_TFEL_MATH_ENZYME_AUTODIFF_HXX
...
namespace tfel::math::enzyme {
   ...
   * \tparam idx: indices of the variables with respect to which the gradients are computed.
  ...
  auto computeGradient(const CallableType&, ArgumentsTypes&&...)  //
  ...
  /*!
   * \brief compute the gradient of a callable with respect to all the variables.
   * \tparam CallableType: type of the callable
   * \tparam ArgumentsTypes: types of the arguments passed to the callable
   * \param[in] c: callable
   * \param[in] args: arguments passed to the callable
  ...
  auto computeGradient(const CallableType&, ArgumentsTypes&&...)         //
      requires((std::is_invocable_v<CallableType, ArgumentsTypes...>)&&  //
               (ScalarConcept<
                   std::invoke_result_t<CallableType, ArgumentsTypes...>>));
  /*!
   * \brief compute the gradient of a free function with respect to the variables
   * designated by the indices `idx`.
   * \tparam idx: indices of the variables with respect to which the gradients are computed.   
   * \tparam F: pointer to the free function
   * \tparam ArgumentsTypes: types of the arguments passed to the free function
   * \param[in] f: free function warpper
   * \param[in] args: arguments passed to the free function
   */
  template <std::size_t... idx,
            internals::IsFunctionPointerConcept auto F,
            typename... ArgumentsTypes>
  auto computeGradient(internals::FunctionWrapper<F>,
                       ArgumentsTypes&&...)  //
      requires((sizeof...(ArgumentsTypes) > 0) &&
               (sizeof...(ArgumentsTypes) < 3) &&  //
               (sizeof...(idx) > 0) &&
  ...
  /*!
   * \brief compute the gradient of a free function with respect to all variables
   * \tparam F: pointer to the free function
   * \tparam ArgumentsTypes: types of the arguments passed to the free function
   * \param[in] f: free function warpper
   * \param[in] args: arguments passed to the free function
...
#include "TFEL/Math/Enzyme/autodiff.ixx"

#endif /* LIB_TFEL_MATH_ENZYME_AUTODIFF_HXX */
